...
public class TimeoutHandler<T> {
     ...
     * @param timeoutBasedOnCpu a boolean.
     * @return a T object.
    ...
    public T execute(final Callable<T> testcase, ExecutorService executor, long timeout,
                     boolean timeoutBasedOnCpu) throws TimeoutException, InterruptedException,
            ExecutionException {
        if (!bean.isCurrentThreadCpuTimeSupported() && timeoutBasedOnCpu) {
            timeoutBasedOnCpu = false;
            logger.warn("Requested to use timeout_based_on_cpu, but it is not supported by the JVM/OS");
        }

        if (!timeoutBasedOnCpu) {
    ...
    private T executeWithCpuBasedTimeout(final Callable<T> testcase,
        ...
        while (waitingTime > 0) {
            try {
                result = task.get(waitingTime, TimeUnit.MILLISECONDS);
                ...
                if (cpuUsage < alpha * timeout) {
                    //CPU has not been used enough
                    waitingTime = timeout - cpuUsage;
                } else {
