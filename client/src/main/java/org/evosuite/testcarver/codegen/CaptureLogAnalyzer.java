...
public final class CaptureLogAnalyzer implements ICaptureLogAnalyzer {
    ...
    @Override
    public void analyze(final CaptureLog originalLog, final ICodeGenerator generator,
                        final Class<?>... observedClasses) {
        this.analyze(originalLog, generator, new HashSet<>(), observedClasses);
    }

    /**
     * Analyzes the given capture log and generates code.
     *
     * @param originalLog     the original log
     * @param generator       the code generator
     * @param blackList       the blacklist of classes
     * @param observedClasses the observed classes
     */
    @SuppressWarnings("rawtypes")
    public void analyze(final CaptureLog originalLog, final ICodeGenerator generator,
                        final Set<Class<?>> blackList, final Class<?>... observedClasses) {
        if (originalLog == null) {
        ...
        CaptureLogAnalyzerException.check(!CollectionUtil.isNullOrEmpty(observedClassNames),
                "could not extract class names for ", Arrays.toString(observedClasses));
    ...
    private int[] analyzeLog(final ICodeGenerator<?> generator,
        ...
        // TODO knowing last logRecNo for termination criterion belonging to an observed instance
        // would prevent processing unnecessary statements
        for (int currentRecord = Math.abs(log.getRecordIndexOfWhereObjectWasInitializedFirst(currentOid));
                currentRecord < numLogRecords; currentRecord++) {
            ...
            logger.debug("Current record {}, current oid {} type {}", currentRecord, currentOid,
                    log.getTypeName(currentOid));
            ...
            if (targetOids.contains(currentOid) && !blackList.contains(getClassFromOID(log, currentOid))) {
                ...
                /*
                 * FIXME: a log object is got as input to be analyzed, but then in the analyzer
                 * there are side effects on it... need re-factoring
                 */
    ...
    /**
     * Finds the caller of the given record.
     *
     * @param log the capture log
     * @param currentRecord the current record
     * @return -1, if there is no caller (very first method call), caller oid otherwise
     */
    private int findCaller(final CaptureLog log, final int currentRecord) {
        ...
        // is not the end of the calling method
        logger.debug("records = {}", record);

        if (record >= numRecords) {
            // did not find any caller -> must be very first method call
            logger.info("[currentRecord={}] - could not find caller for currentRecord -> "
                            + "must be very first method call",
                    currentRecord);
            return -1;
        } else {
    ...
    private int findEndOfMethod(final CaptureLog log, final int currentRecord, final int currentOid) {
        ...
        while (true) {
            ...
            logger.debug("Current record: {}: {} <-> {}, {} <-> {}", record, captureId, log.captureIds.get(record),
                    currentOid, log.objectIds.get(record));
    ...
    @SuppressWarnings({"rawtypes"})
    private void restoreArgs(final Object[] args, final int currentRecord, final CaptureLog log,
                             final ICodeGenerator generator, final Set<Class<?>> blackList) {
        Integer oid;
    ...
    @SuppressWarnings({"rawtypes"})
    private int[] restoreCodeFromLastPosTo(final CaptureLog log, final ICodeGenerator generator, final int oid,
                                          final int end, final Set<Class<?>> blackList) {
        ...
        for (; currentRecord < end; currentRecord++) {
            ...
            logger.debug("Checking: " + currentRecord + ": " + log.getTypeName(currentOid)
                    + " to generate " + log.getTypeName(oid));
            ...
            if (oid == currentOid || returnValue == oid) {
                ...
                if (oid != currentOid) {
                    ...
                    // currentOid differs to the targetOID. this happens if the targetOID appears
                    // the first time as return value -> so we have to make sure that currentOid
                    // is restored till this position in order to deliver the correct
                    // target instance
                ...
                if (CaptureLog.PLAIN_INIT.equals(methodName)) {
                    ...
                    if (dependencyOid != CaptureLog.NO_DEPENDENCY) {
                        exchange = this.restoreCodeFromLastPosTo(log, generator, dependencyOid,
                                currentRecord, blackList);
                        if (exchange != null) {
                            return exchange;
                        }
                    }
                    ...
                    currentRecord = findEndOfMethod(log, currentRecord, currentOid);
                } else if (CaptureLog.PUTFIELD.equals(methodName) || CaptureLog.PUTSTATIC.equals(methodName)
                        || CaptureLog.GETFIELD.equals(methodName) || CaptureLog.GETSTATIC.equals(methodName)) {
                    // field write access such as p.id = id or Person.staticVar = "something"
                    // field READ access such as "int a =  p.id" or "String var = Person.staticVar"
                    ...
                    if (CaptureLog.PUTFIELD.equals(methodName) || CaptureLog.PUTSTATIC.equals(methodName)) {
                        ...
                        if (methodArgOid != null && methodArgOid != oid) {
                            ...
                            exchange = this.restoreCodeFromLastPosTo(log, generator, methodArgOid,
                                    currentRecord, blackList);
                            if (exchange != null) {
                                return exchange;
                            }
                    ...
                    if (CaptureLog.GETFIELD.equals(methodName) || CaptureLog.GETSTATIC.equals(methodName)) {
                        // GETFIELD and GETSTATIC should only happen, if we obtain an instance
                        // whose creation has not been observed
                    ...
                    if (dependencyOid != CaptureLog.NO_DEPENDENCY) {
                        ...
                        exchange = this.restoreCodeFromLastPosTo(log, generator, dependencyOid, currentRecord,
                                blackList);
                        if (exchange != null) {
                            return exchange;
                        }
                    ...
                    methodArgs = log.params.get(currentRecord);
                    logger.debug("Getting " + methodArgs.length + " method args: {}", (Object) methodArgs);
                    for (final Object methodArg : methodArgs) {
                        ...
                        // if method argument is equal to caller oid  look for alternative instance
                        // which is restorable
                        if (methodArgOid != null && (methodArgOid == callerOid)) {
                            ...
                            logger.debug("arg in blacklist >>>> {}",
                                    blackList.contains(this.getClassFromOID(log, methodArgOid)));
                        ...
                        if (methodArgOid != null && methodArgOid != oid) {
                            ...
                            exchange = this.restoreCodeFromLastPosTo(log, generator, methodArgOid, currentRecord,
                                    blackList);
                            if (exchange != null) {
                    ...
                    if (isBlackListed(currentOid, blackList, log)) {
                        logger.debug("-> is blacklisted... " + blackList + " oid: " + currentOid
                                + " class: " + getClassFromOID(log, currentOid));
                    ...
                    if (returnValue != -1) {
                        // if returnValue has not type VOID, mark current log record as record where
                        // the return value instance was created --> if an object is created within
                        // an observed method, it would not be semantically correct (and impossible
                        // to handle properly) to create an extra instance of the return value type
                        // outside this method
                        this.updateInitRec(log, returnValue, currentRecord);
                    }
    ...
    private int handleArrayInit(final CaptureLog log,
        ...
        try {
            ...
            CaptureLogAnalyzerException.propagateError(e,
                    "[currentRecord = %s, currentOid = %s, blackList = %s] - an error occurred while "
                            + "creating array init stmt\n",
    ...
    private int[] getExchange(final CaptureLog log, final int currentRecord, final int oid,
                              final Set<Class<?>> blackList) {
        int callerOid;
        int r = currentRecord;
        ...
        //   blackList.contains(this.getClassFromOID(log, callerOid)));
