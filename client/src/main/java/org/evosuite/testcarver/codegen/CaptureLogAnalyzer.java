...
public final class CaptureLogAnalyzer implements ICaptureLogAnalyzer {
    ...
    @SuppressWarnings("rawtypes")
    public void analyze(final CaptureLog originalLog, final ICodeGenerator generator, final Set<Class<?>> blackList,
                        final Class<?>... observedClasses) {
        if (originalLog == null) {
        ...
        if (targetOids.isEmpty()) {
            logger.info("could not find any oids for {} -> {} ==> no code is generated\n", observedClassNames,
                    Arrays.toString(observedClasses));
            return;
        } else {
    ...
    private int[] analyzeLog(final ICodeGenerator<?> generator,
        ...
        // TODO knowing last logRecNo for termination criterion belonging to an observed instance would prevent
        //  processing unnecessary statements
        for (int currentRecord = Math.abs(log.getRecordIndexOfWhereObjectWasInitializedFirst(currentOid));
            ...
            if (targetOids.contains(currentOid) && !blackList.contains(getClassFromOID(log, currentOid))) {
                ...
                /*
                 * FIXME: a log object is got as input to be analyzed, but then in the analyzer there are side
                 *  effects on it... need re-factoring
                 */
    ...
    private Class<?> getClassForName(String type) {
        try {
            if (type.equals("boolean")) {
                ...
                return Short.TYPE;
            } else if (type.equals("String") || type.equals("Boolean") || type.equals("Short")
                    || type.equals("Long") || type.equals("Integer") || type.equals("Float")
    ...
    /**
     * Finds the caller of the current record.
     *
     * @param log           the log
    ...
    private int findCaller(final CaptureLog log, final int currentRecord) {
        ...
        if (record >= numRecords) {
            ...
            logger.info(
                    "[currentRecord={}] - could not find caller for currentRecord -> must be very first method call",
                    currentRecord);
            return -1;
        } else {
    ...
    private int findEndOfMethod(final CaptureLog log, final int currentRecord, final int currentOid) {
        ...
        while (true) {
            ...
            logger.debug("Current record: {}: {} <-> {}, {} <-> {}", record, captureId,
                    log.captureIds.get(record), currentOid, log.objectIds.get(record));
        ...
        for (; currentRecord < end; currentRecord++) {
            ...
            if (oid == currentOid || returnValue == oid) {
                ...
                if (oid != currentOid) {
                    ...
                    // currentOID differs to the targetOID. this happens if the targetOID appears the first time as
                    // return value -> so we have to make sure that currentOID is restored till this position in order
                    // to deliver the correct target instance
                ...
                if (CaptureLog.PLAIN_INIT.equals(methodName)) {
                    ...
                    if (CaptureLog.GETFIELD.equals(methodName) || CaptureLog.GETSTATIC.equals(methodName)) {
                        // GETFIELD and GETSTATIC should only happen, if we obtain an instance whose creation
                    ...
                    if (isBlackListed(currentOid, blackList, log)) {
                        logger.debug("-> is blacklisted... " + blackList + " oid: " + currentOid + " class: "
                                + getClassFromOID(log, currentOid));
                    ...
                    if (returnValue != -1) {
                        // if returnValue has not type VOID, mark current log record as record where the return value
                        // instance was created
                        // --> if an object is created within an observed method, it would not be semantically correct
                        //     (and impossible to handle properly) to create an extra instance of the return value type
                        //     outside this method
                        this.updateInitRec(log, returnValue, currentRecord);
                    }
    ...
    private int handleArrayInit(final CaptureLog log,
        ...
        try {
            ...
            CaptureLogAnalyzerException.propagateError(e,
                    "[currentRecord = %s, currentOID = %s, blackList = %s] - an error occurred while creating "
                            + "array init stmt\n",
