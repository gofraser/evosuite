...
public class PathConditionUtils {
    ...
    public static final String TOTAL_AMOUNT_OF_PATHS_HAS_TO_BE_HIGHER_THAN_0 =
            "Total amount of paths has to be higher than 0.";
    public static final String EXPECTED_PREFIX_PATH_CONDITION_CANNOT_BE_NULL =
            "Expected prefix path condition cannot be null.";

    /**
     * Returns true if the constraints in the query are a supra set of any of the constraints in the set
     * of queries.
     *
     * @param query   the set of constraints to check.
     * @param queries the collection of constraint sets to compare against.
     * @return true if query is a supra set of any set in queries.
     */
    public static boolean isConstraintSetSupraSetOf(Set<Constraint<?>> query, Set<Set<Constraint<?>>> queries) {
        if (query == null) {
            throw new IllegalArgumentException(QUERY_CANNOT_BE_NULL_EXCEPTION_MESSAGE);
        }
        if (queries == null) {
            throw new IllegalArgumentException(QUERIES_CANNOT_BE_NULL_EXCEPTION_MESSAGE);
        }
     ...
     * @param query   the set of constraints to check.
     * @param queries the collection of constraint sets to compare against.
     * @return the supra set if found.
     */
    public static Set<Constraint<?>> getConstraintSetSubSetOf(Set<Constraint<?>> query,
     ...
     * @return the supra set if found.
     */
    public static Set<Constraint<?>> getConstraintSetSupraSetOf(Set<Constraint<?>> query,
                                                                Collection<Set<Constraint<?>>> queries) {
        if (query == null) {
            throw new IllegalArgumentException(QUERY_CANNOT_BE_NULL_EXCEPTION_MESSAGE);
        }
     ...
     *
     * <p>We simply check that the original path condition from which the new test was created
     * is a prefix of the actual path being explored.
     *
     * <p>TODO: see how to cite this paper properly.
     * Idea taken from: An empirical investigation into path divergences for concolic execution using CREST,
     * Ting Chen, Xiaodong Lin, Jin Huang, Abel Bacchus and Xiaosong Zhang.
     *
     * <p>TODO: In the future we can support explaining which condition diverged.
     ...
     *
     * @param expectedPrefixPathCondition the original path condition.
     * @param newPathCondition            the actual path condition.
     * @return true if the path condition has diverged.
     */
    public static boolean hasPathConditionDiverged(PathCondition expectedPrefixPathCondition,
                                                   PathCondition newPathCondition) {
        if (expectedPrefixPathCondition == null) {
            throw new IllegalArgumentException(EXPECTED_PREFIX_PATH_CONDITION_CANNOT_BE_NULL);
        }

        if (newPathCondition == null) {
            throw new IllegalArgumentException(NEW_PATH_CONDITION_CANNOT_BE_NULL);
        }
        ...
        if (expectedPrefixPathCondition.size() > newBranchConditions.size()) {
            return true;
        }

        for (int i = 0; i < expectedPrefixBranchConditions.size(); ++i) {
            BranchCondition expectedPrefixBranchCondition = expectedPrefixBranchConditions.get(i);
            BranchCondition newBranchCondition = newBranchConditions.get(i);
     ...
     *
     * @param pathDivergeAmount the number of diverged paths.
     * @param totalPathsAmount  the total number of paths.
     * @return the ratio of diverged paths to total paths.
     */
    public static double calculatePathDivergenceRatio(int pathDivergeAmount, int totalPathsAmount) {
        if (totalPathsAmount == 0) {
            throw new ArithmeticException(TOTAL_AMOUNT_OF_PATHS_HAS_TO_BE_HIGHER_THAN_0);
        }
