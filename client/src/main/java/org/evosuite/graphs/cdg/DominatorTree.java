...
public class DominatorTree<V> extends EvoSuiteGraph<DominatorNode<V>, DefaultEdge> {
    ...
    private void computeDominatorFrontiers(DominatorNode<V> currentNode) {
        ...
        for (DominatorNode<V> z : getChildren(currentNode)) {
            ...
            if (childDF != null) {
                for (V y : childDF) {
                    DominatorNode<V> dominatorNodeY = getDominatorNodeFor(y);
                    if (dominatorNodeY.immediateDominator == null
                            || dominatorNodeY.immediateDominator.dfsNumber != currentNode.dfsNumber) {
    ...
    private void depthFirstAnalyze(DominatorNode<V> currentNode) {
        ...
        for (V w : cfg.getChildren(currentNode.node)) {
            DominatorNode<V> nodeW = getDominatorNodeFor(w);
            if (nodeW.semiDominator == null) {
                nodeW.parent = currentNode;
                depthFirstAnalyze(nodeW);
            }
        }
    }
