...
public class Scaffolding {
    ...
    /** Canonical fixture inputs captured before oracle post-processing. */
    public static final class ReplaySnapshot {
        private final List<String> classesToReset;
        private final List<String> classesToInitialize;
        private final List<String> mockClasses;
        private final RuntimeRequirements runtimeRequirements;

        ReplaySnapshot(List<String> classesToReset,
                       List<String> classesToInitialize,
                       List<String> mockClasses) {
            this(classesToReset, classesToInitialize, mockClasses,
                    RuntimeRequirements.fromResults(Collections.emptyList()));
        }

        private ReplaySnapshot(List<String> classesToReset,
                               List<String> classesToInitialize,
                               List<String> mockClasses,
                               RuntimeRequirements runtimeRequirements) {
            this.classesToReset = new ArrayList<>(classesToReset);
            this.classesToInitialize = new ArrayList<>(classesToInitialize);
            this.mockClasses = new ArrayList<>(mockClasses);
            this.runtimeRequirements = runtimeRequirements;
        }

        RuntimeRequirements getRuntimeRequirements() {
            return runtimeRequirements;
        }
    }

    /**
     * Capture every mutable input used to render scaffolding. Oracle
     * post-processing executes observer code and can otherwise change the
     * instrumented-class registry or mock preload set of only one arm.
     */
    public static ReplaySnapshot captureReplaySnapshot(
            List<TestCase> tests, List<ExecutionResult> results) {
        List<String> classesToReset =
                ClassReInitializer.getInstance().getInitializedClasses();
        List<String> classesToInitialize = TestGenerationContext.getInstance()
                .getClassLoaderForSUT().getViewOfInstrumentedClasses();
        Set<String> mockClasses = new LinkedHashSet<>();
        for (TestCase test : tests) {
            for (Statement statement : test) {
                if (statement instanceof FunctionalMockStatement) {
                    FunctionalMockStatement mock = (FunctionalMockStatement) statement;
                    mockClasses.add(GenericClassFactory.get(mock.getReturnType())
                            .getRawClass().getTypeName());
                }
            }
        }
        return new ReplaySnapshot(classesToReset, classesToInitialize,
                new ArrayList<>(mockClasses), RuntimeRequirements.fromResults(results));
    }
    ...
    public static String getScaffoldingFileContent(String testName, List<ExecutionResult> results,
        ...
        return getScaffoldingFileContent(testName, results, requirements, null);
    }

    /**
     * Return scaffolding while using a fixed class-reset snapshot. Replay
     * callers use this to keep assertion observation and validation from
     * changing the fixture shared by paired oracle arms.
     */
    static String getScaffoldingFileContent(String testName, List<ExecutionResult> results,
                                            RuntimeRequirements requirements,
        ...
        if (results.isEmpty()) {
            ...
            builder.append(new Scaffolding().getBeforeAndAfterMethods(name,
                    requirements.hasSecurityException(), results, replaySnapshot));
        }
    ...
    protected static String getHeader(String name, List<ExecutionResult> results, RuntimeRequirements requirements) {
        ...
        builder.append(" * Generation timestamp omitted for deterministic output\n");
        builder.append(" */\n\n");
    ...
    public String getBeforeAndAfterMethods(String name, boolean wasSecurityException, List<ExecutionResult> results) {
        return getBeforeAndAfterMethods(name, wasSecurityException, results, null);
    }

    String getBeforeAndAfterMethods(String name, boolean wasSecurityException,
        ...
        generateInitializeClasses(name, bd, replaySnapshot);

        generateMockInitialization(name, bd, results, replaySnapshot);

        if (Properties.RESET_STATIC_FIELDS) {
            generateResetClasses(name, bd, replaySnapshot);
        }
     ...
     * @param results execution results
     */
    private void generateMockInitialization(String testClassName, StringBuilder bd,
                                              List<ExecutionResult> results,
                                              ReplaySnapshot replaySnapshot) {
        if (replaySnapshot == null && !TestSuiteWriterUtils.doesUseMocks(results)) {
            return;
        }
        if (replaySnapshot != null && replaySnapshot.mockClasses.isEmpty()) {
            return;
        }
        ...
        if (replaySnapshot == null) {
            ...
            for (String name : replaySnapshot.mockClasses) {
                mockStatements.add("mock(Class.forName(\"" + name + "\", false, "
                        + testClassName + ".class.getClassLoader()));");
            }
        }
    ...
    private void generateResetClasses(String testClassName, StringBuilder bd,
                                      ReplaySnapshot replaySnapshot) {
        List<String> classesToReset = replaySnapshot == null
                ? ClassReInitializer.getInstance().getInitializedClasses()
                : replaySnapshot.classesToReset;
    ...
    private void generateInitializeClasses(String testClassName, StringBuilder bd,
        ...
        List<String> allInstrumentedClasses = replaySnapshot == null
                ? TestGenerationContext.getInstance().getClassLoaderForSUT()
                        .getViewOfInstrumentedClasses()
                : replaySnapshot.classesToInitialize;
