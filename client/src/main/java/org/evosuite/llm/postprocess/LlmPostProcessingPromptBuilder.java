...
final class LlmPostProcessingPromptBuilder {
    ...
    private static String userPrompt(LlmPostProcessingPromptContext context, int testIndex,
        ...
        builder.append("- When variableNames is enabled, propose names for every nameable vN whose current rendered name is generic, type-only, or numeric-suffixed.\n");
        ...
        builder.append("- For multiple variables of the same type, choose distinct role names such as gui, importEvent, dispatchedEvent, expectedCount, actualCount, sourcePath, or targetPath; keep a numeric suffix only as a last resort to avoid a real collision.\n");
